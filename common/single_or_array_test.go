...
type singleOrArrayCase[T any] struct {
	name     string
	data     []byte
	expected *common.SingleOrArray[T]
	wantErr  bool
}
		...
		testSingleOrArrayJSON(t, []singleOrArrayCase[string]{
			{
				name:     "single",
				data:     []byte(`"single"`),
				expected: common.NewSingleOrArray("single"),
			},
			{
				name:     "multi",
				data:     []byte(`["first","second"]`),
				expected: common.NewSingleOrArray("first", "second"),
			},
			{
				name: "null",
				data: []byte(`null`),
		...
		testSingleOrArrayJSON(t, []singleOrArrayCase[int]{
			{
				name:     "single",
				data:     []byte(`1`),
				expected: common.NewSingleOrArray(1),
			},
			{
				name:     "multi",
				data:     []byte(`[1,2]`),
				expected: common.NewSingleOrArray(1, 2),
			},
			{
				name: "null",
				data: []byte(`null`),
		...
		testSingleOrArrayJSON(t, []singleOrArrayCase[Foo]{
			{
				...
				expected: common.NewSingleOrArray(Foo{A: "single", B: 42}),
			},
			{
				name:     "multi",
				data:     []byte(`[{"A":"first","B":1},{"A":"second","B":2}]`),
				expected: common.NewSingleOrArray(Foo{A: "first", B: 1}, Foo{A: "second", B: 2}),
			},
			{
				name: "null",
				data: []byte(`null`),
