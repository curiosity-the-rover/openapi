...
type OpenAPI struct {
	...
	Components *common.Extendable[Components] `json:"components,omitempty" yaml:"components,omitempty"`
	// REQUIRED
	// Provides metadata about the API. The metadata MAY be used by tooling as required.
	Info *common.Extendable[Info] `json:"info" yaml:"info"`
	...
	Paths *common.Extendable[Paths] `json:"paths,omitempty" yaml:"paths,omitempty"`
	...
	Tags []*common.Extendable[Tag] `json:"tags,omitempty" yaml:"tags,omitempty"`
	// An array of Server Objects, which provide connectivity information to a target server.
	// If the servers property is not provided, or is an empty array, the default value would be a Server Object with a url value of /.
	Servers []*common.Extendable[Server] `json:"servers,omitempty" yaml:"servers,omitempty"`
}

func checkUnusedComponent[T any](name string, m map[string]T, validator *common.Validator) []*common.validationError {
	var errs []*common.validationError
	for k := range m {
		id := common.joinLoc("#", "components", name, k)
		if !validator.visited[id] {
			errs = append(errs, common.newValidationError(id, common.ErrUnused))
		}
	}
	return errs
}
...
func (o *OpenAPI) validateSpec(location string, validator *common.Validator) []*common.validationError {
	var errs []*common.validationError
	if o.OpenAPI == "" {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "openapi"), common.ErrRequired))
	} else if !strings.HasPrefix(o.OpenAPI, "3.1.") {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "openapi"), NewUnsupportedVersionError(o.OpenAPI)))
	}
	if o.Info == nil {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "info"), common.ErrRequired))
	} else {
		errs = append(errs, o.Info.validateSpec(common.joinLoc(location, "info"), validator)...)
	}

	// validate tags first to memorize them for later checking
	if o.Tags != nil {
		for i, tag := range o.Tags {
			errs = append(errs, tag.validateSpec(common.joinLoc(location, "tag", i), validator)...)
		}
	}

	if err := common.checkURL(o.JsonSchemaDialect); err != nil {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "jsonSchemaDialect"), err))
	}
	if o.Servers != nil {
		for i, server := range o.Servers {
			errs = append(errs, server.validateSpec(common.joinLoc(location, "servers", i), validator)...)
		}
	}
	if o.Paths != nil {
		errs = append(errs, o.Paths.validateSpec(common.joinLoc(location, "paths"), validator)...)
	}
	if o.WebHooks != nil {
		for name, webhook := range o.WebHooks {
			errs = append(errs, webhook.validateSpec(common.joinLoc(location, "webhooks", name), validator)...)
		}
	}
	if o.Components != nil {
		errs = append(errs, o.Components.validateSpec(common.joinLoc(location, "components"), validator)...)
	}
	if o.Security != nil {
		for i, security := range o.Security {
			errs = append(errs, security.validateSpec(common.joinLoc(location, "security", i), validator)...)
		}
	}
	...
	if o.Paths == nil && o.WebHooks == nil && o.Components == nil {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "paths||webhooks||components"), common.ErrRequired))
	}

	// check for unused
	if !validator.opts.allowUnusedTags {
		for i, t := range o.Tags {
			if !validator.visited[common.joinLoc("tags", t.Spec.Name, "used")] {
				errs = append(errs, common.newValidationError(common.joinLoc(location, "tags", i), fmt.Errorf("'%s': %w", t.Spec.Name, common.ErrUnused)))
			}
		}
	}
	...
	for k, v := range validator.linkToOperationID {
		if !validator.visited[common.joinLoc("operations", v)] {
			errs = append(errs, common.newValidationError(k, "'%s' not found", v))
		}
	}
	return errs
}
...
type OpenAPIBuilder struct {
	spec *common.Extendable[OpenAPI]
}

func NewOpenAPIBuilder() *OpenAPIBuilder {
	return &OpenAPIBuilder{spec: common.NewExtendable(&OpenAPI{
		OpenAPI:           "3.1.1",
		JsonSchemaDialect: "https://spec.openapis.org/oas/3.1/dialect/base",
	})}
}

func (b *OpenAPIBuilder) Build() *common.Extendable[OpenAPI] {
	return b.spec
}
...
func (b *OpenAPIBuilder) Info(info *common.Extendable[Info]) *OpenAPIBuilder {
	b.spec.Spec.Info = info
	return b
}

func (b *OpenAPIBuilder) Components(components *common.Extendable[Components]) *OpenAPIBuilder {
...
func (b *OpenAPIBuilder) ExternalDocs(externalDocs *common.Extendable[ExternalDocs]) *OpenAPIBuilder {
	b.spec.Spec.ExternalDocs = externalDocs
	return b
}

func (b *OpenAPIBuilder) Paths(paths *common.Extendable[Paths]) *OpenAPIBuilder {
	b.spec.Spec.Paths = paths
	return b
}

func (b *OpenAPIBuilder) AddPath(path string, item *common.RefOrSpec[common.Extendable[PathItem]]) *OpenAPIBuilder {
...
func (b *OpenAPIBuilder) AddWebHook(name string, path *common.RefOrSpec[common.Extendable[PathItem]]) *OpenAPIBuilder {
...
func (b *OpenAPIBuilder) Tags(tags ...*common.Extendable[Tag]) *OpenAPIBuilder {
	b.spec.Spec.Tags = tags
	return b
}

func (b *OpenAPIBuilder) AddTags(tags ...*common.Extendable[Tag]) *OpenAPIBuilder {
	b.spec.Spec.Tags = append(b.spec.Spec.Tags, tags...)
	return b
}

func (b *OpenAPIBuilder) Servers(servers ...*common.Extendable[Server]) *OpenAPIBuilder {
	b.spec.Spec.Servers = servers
	return b
}

func (b *OpenAPIBuilder) AddServers(servers ...*common.Extendable[Server]) *OpenAPIBuilder {
