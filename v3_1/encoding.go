...
func (o *Encoding) validateSpec(location string, validator *common.Validator) []*common.validationError {
	...
	default:
		errs = append(errs, common.newValidationError(common.joinLoc(location, "style"), "invalid value, expected one of [%s, %s, %s, %s], but got '%s'", StyleForm, StyleSpaceDelimited, StylePipeDelimited, StyleDeepObject, o.Style))
	}
	return errs
}

type EncodingBuilder struct {
	spec *common.Extendable[Encoding]
}

func NewEncodingBuilder() *EncodingBuilder {
	return &EncodingBuilder{
		spec: common.NewExtendable[Encoding](&Encoding{}),
	}
}

func (b *EncodingBuilder) Build() *common.Extendable[Encoding] {
	return b.spec
}
...
func (b *EncodingBuilder) Headers(v map[string]*common.RefOrSpec[common.Extendable[Header]]) *EncodingBuilder {
	b.spec.Spec.Headers = v
	return b
}

func (b *EncodingBuilder) Header(name string, value *common.RefOrSpec[common.Extendable[Header]]) *EncodingBuilder {
	if b.spec.Spec.Headers == nil {
		b.spec.Spec.Headers = make(map[string]*common.RefOrSpec[common.Extendable[Header]], 1)
	}
