...
func (o *Header) validateSpec(location string, validator *common.Validator) []*common.validationError {
	...
	if l := len(o.Content); l > 0 {
		if l != 1 {
			errs = append(errs, common.newValidationError(common.joinLoc(location, "content"), "must be only one item, but got '%d'", l))
		}
	...
	case "", StyleSimple:
	default:
		errs = append(errs, common.newValidationError(common.joinLoc(location, "style"), "invalid value, expected one of [%s], but got '%s'", StyleSimple, o.Style))
	}
...
type HeaderBuilder struct {
	spec *common.RefOrSpec[common.Extendable[Header]]
}

func NewHeaderBuilder() *HeaderBuilder {
	return &HeaderBuilder{
		spec: common.NewRefOrExtSpec[Header](&Header{}),
	}
}

func (b *HeaderBuilder) Build() *common.RefOrSpec[common.Extendable[Header]] {
	return b.spec
}
...
func (b *HeaderBuilder) Schema(v *common.RefOrSpec[Schema]) *HeaderBuilder {
	b.spec.Spec.Spec.Schema = v
	return b
}

func (b *HeaderBuilder) Content(v map[string]*common.Extendable[MediaType]) *HeaderBuilder {
	b.spec.Spec.Spec.Content = v
	return b
}

func (b *HeaderBuilder) AddContent(name string, value *common.Extendable[MediaType]) *HeaderBuilder {
