...
type Responses struct {
	...
	Default *common.RefOrSpec[common.Extendable[Response]] `json:"default,omitempty" yaml:"default,omitempty"`
	...
	Response map[string]*common.RefOrSpec[common.Extendable[Response]] `json:"-" yaml:"-"`
}
...
func (o *Responses) validateSpec(location string, validator *common.Validator) []*common.validationError {
	var errs []*common.validationError
	if o.Default != nil {
		errs = append(errs, o.Default.validateSpec(common.joinLoc(location, "default"), validator)...)
	}
	for k, v := range o.Response {
		if !ResponseCodePattern.MatchString(k) {
			errs = append(errs, common.newValidationError(common.joinLoc(location, k), "must match pattern '%s', but got '%s'", ResponseCodePattern, k))
		}
...
type ResponsesBuilder struct {
	spec *common.RefOrSpec[common.Extendable[Responses]]
}

func NewResponsesBuilder() *ResponsesBuilder {
	return &ResponsesBuilder{
		spec: common.NewRefOrExtSpec[Responses](&Responses{}),
	}
}

func (b *ResponsesBuilder) Build() *common.RefOrSpec[common.Extendable[Responses]] {
	return b.spec
}
...
func (b *ResponsesBuilder) Default(v *common.RefOrSpec[common.Extendable[Response]]) *ResponsesBuilder {
	b.spec.Spec.Spec.Default = v
	return b
}

func (b *ResponsesBuilder) Response(v map[string]*common.RefOrSpec[common.Extendable[Response]]) *ResponsesBuilder {
	b.spec.Spec.Spec.Response = v
	return b
}

func (b *ResponsesBuilder) AddResponse(key string, value *common.RefOrSpec[common.Extendable[Response]]) *ResponsesBuilder {
	if b.spec.Spec.Spec.Response == nil {
		b.spec.Spec.Spec.Response = make(map[string]*common.RefOrSpec[common.Extendable[Response]], 1)
	}
