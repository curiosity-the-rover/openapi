...
func (o *Response) validateSpec(location string, validator *common.Validator) []*common.validationError {
	errs := make([]*common.validationError, 0)
	if o.Description == "" {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "description"), common.ErrRequired))
	}
	if o.Content != nil {
...
type ResponseBuilder struct {
	spec *common.RefOrSpec[common.Extendable[Response]]
}

func NewResponseBuilder() *ResponseBuilder {
	return &ResponseBuilder{
		spec: common.NewRefOrExtSpec[Response](&Response{}),
	}
}

func (b *ResponseBuilder) Build() *common.RefOrSpec[common.Extendable[Response]] {
	return b.spec
}
...
func (b *ResponseBuilder) Headers(v map[string]*common.RefOrSpec[common.Extendable[Header]]) *ResponseBuilder {
	b.spec.Spec.Spec.Headers = v
	return b
}

func (b *ResponseBuilder) AddHeader(key string, value *common.RefOrSpec[common.Extendable[Header]]) *ResponseBuilder {
	if b.spec.Spec.Spec.Headers == nil {
		b.spec.Spec.Spec.Headers = make(map[string]*common.RefOrSpec[common.Extendable[Header]], 1)
	}
	b.spec.Spec.Spec.Headers[key] = value
	return b
}

func (b *ResponseBuilder) Content(v map[string]*common.Extendable[MediaType]) *ResponseBuilder {
	b.spec.Spec.Spec.Content = v
	return b
}

func (b *ResponseBuilder) AddContent(key string, value *common.Extendable[MediaType]) *ResponseBuilder {
...
func (b *ResponseBuilder) Links(v map[string]*common.RefOrSpec[common.Extendable[Link]]) *ResponseBuilder {
	b.spec.Spec.Spec.Links = v
	return b
}

func (b *ResponseBuilder) AddLink(key string, value *common.RefOrSpec[common.Extendable[Link]]) *ResponseBuilder {
	if b.spec.Spec.Spec.Links == nil {
		b.spec.Spec.Spec.Links = make(map[string]*common.RefOrSpec[common.Extendable[Link]], 1)
	}
