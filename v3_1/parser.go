...
func ParseObject(obj any, components *common.Extendable[Components], opts ...ParseOption) (*SchemaBuilder, error) {
	opt := &parseOptions{
	...
	return parseObject(common.joinLoc("", t.String()), t, components, opt)
}
...
func parseObject(location string, t reflect.Type, components *common.Extendable[Components], opt *parseOptions) (*SchemaBuilder, error) {
	if t == nil {
		return NewSchemaBuilder().Type(NullType).GoType("nil"), nil
	}
	kind := t.Kind()
	...
	default:
		...
		case reflect.Array, reflect.Slice:
			...
			builder.Type(ArrayType).Items(common.NewBoolOrSchema(elemSchema)).GoType("")
		case reflect.Map:
			...
			builder.Type(ObjectType).AdditionalProperties(common.NewBoolOrSchema(elemSchema)).GoType("")
		case reflect.Struct:
			...
			if components.Spec.Schemas == nil {
				components.Spec.Schemas = make(map[string]*common.RefOrSpec[Schema], 1)
			}
			...
			var allOf []*common.RefOrSpec[Schema]
			for i := range t.NumField() {
				...
				fieldSchema, err := parseObject(common.joinLoc(location, field.Name), obj.Field(i).Type(), components, opt)
				if err != nil {
