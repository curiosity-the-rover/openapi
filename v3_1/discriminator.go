...
func (o *Discriminator) validateSpec(location string, validator *common.Validator) []*common.validationError {
	var errs []*common.validationError
	if o.PropertyName == "" {
		errs = append(errs, common.newValidationError(common.joinLoc(location, "propertyName"), common.ErrRequired))
	}
	for k, v := range o.Mapping {
		ref := common.NewRefOrSpec[Schema](v)
		errs = append(errs, ref.validateSpec(common.joinLoc(location, "mapping", k), validator)...)
	}
	return errs
}
